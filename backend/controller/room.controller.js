...
// Get room stats (by condition)
export const getRoomStats = async (req, res) => {
  try {
    const total = await Room.countDocuments();
    const good = await Room.countDocuments({ condition: "Good" });
    const damaged = await Room.countDocuments({ condition: "Damaged" });
    res.json({ total, good, damaged });
  } catch (err) {
